...
import { Calendar, Users, Phone, Upload, Settings, PieChart, LogOut } from 'lucide-react';
...
const DialerSystem = () => {
  ...
  useEffect(() => {
    ...
    if (savedToken) {
      ...
      // Instead of creating a separate endpoint to get user data,
      // we can decode the JWT token to get basic info
      try {
        ...
        // Fetch tenant data
        fetchTenantData(decodedToken.tenantId, savedToken);
      } catch (error) {
    ...
    const handleToggleDID = async (id, currentActive) => {
      try {
        ...
        setDids(dids.map(did => 
          did.id === id ? { ...did, isActive: !currentActive } : did
        ));
      } catch (error) {
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {filteredDids.map(did => (
                        <tr key={did.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <button
                              onClick={() => handleToggleDID(did.id, did.isActive)}
