...
// QUICK FIX: In your server.js, change the Lead model definition:
...
const Lead = sequelize.define('Lead', {
  ...
  status: {
    type: DataTypes.STRING,  // ✅ Changed from DataTypes.ENUM
...
// Update lead dialer assignment
router.put('/api/leads/:id/dialer-assignment', authenticateToken, async (req, res) => {
  try {
    const { dialerAssignment } = req.body;
    const leadId = req.params.id;
    const tenantId = req.user.tenantId;
    
    if (!['auto_dialer', 'journey_only', 'both', 'none'].includes(dialerAssignment)) {
      return res.status(400).json({ error: 'Invalid dialer assignment value' });
    }
    ...
    res.json({
      message: 'Dialer assignment updated',
      leadId,
      dialerAssignment
    });
  } catch (error) {
...
router.post('/api/leads/bulk-dialer-assignment', authenticateToken, async (req, res) => {
  try {
    const { leadIds, dialerAssignment } = req.body;
    const tenantId = req.user.tenantId;
    ...
    const [updateCount] = await Lead.update(
      { dialerAssignment },
      {
        where: {
          id: { [Op.in]: leadIds },
    ...
    res.json({
      message: `Updated ${updateCount} leads`,
      dialerAssignment
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
...
router.get('/api/leads/dialer-stats', authenticateToken, async (req, res) => {
  try {
    const tenantId = req.user.tenantId;
    ...
    try {
      ...
      const leads = results.map(row => {
        ...
        if (row.brand || row.Brand) {
          lead.brand = row.brand || row.Brand;
        }
        
        if (row.source || row.Source) {
          lead.source = row.source || row.Source;
        }

        Object.keys(row).forEach(key => {
          if (!['phone', 'Phone', 'PhoneNumber', 'phone_number', 
      ...
      // Auto-enroll leads in journeys with auto-enroll enabled
      if (journeyModels && journeyService) {
        try {
          // Use proper JSONB query syntax for PostgreSQL
          const autoEnrollJourneys = await journeyModels.Journey.findAll({
            where: {
              tenantId,
              isActive: true,
              [Op.and]: [
                sequelize.literal(`"triggerCriteria"->>'autoEnroll' = 'true'`)
              ]
            }
          });
          
          if (autoEnrollJourneys.length > 0) {
            console.log(`Found ${autoEnrollJourneys.length} auto-enroll journeys for tenant ${tenantId}`);
            
            // Get the created leads
            const createdLeads = await Lead.findAll({
              where: {
                phone: {
                  [Op.in]: validLeads.map(lead => lead.phone)
                },
                tenantId
              },
            ...
            console.log(`Processing ${createdLeads.length} leads for auto-enrollment`);
            
            // Process auto-enrollment asynchronously to not block the response
              ...
              try {
                ...
                for (const journey of autoEnrollJourneys) {
                  ...
                  for (const lead of createdLeads) {
                    try {
                      // Check if lead matches journey criteria
                      const matchesCriteria = await matchesJourneyCriteria(lead, journey.triggerCriteria);
                      
                      if (matchesCriteria) {
                        // Check if lead is already enrolled in this journey
                        const existingEnrollment = await journeyModels.LeadJourney.findOne({
                          where: {
                            leadId: lead.id,
                        ...
                        if (!existingEnrollment) {
                          ...
                          console.log(`Auto-enrolled lead ${lead.id} in journey ${journey.id} (${journey.name})`);
                          journeyEnrolled++;
                          totalEnrolled++;
                        } else {
                          console.log(`Lead ${lead.id} already enrolled in journey ${journey.id}`);
                        }
                      } else {
                        console.log(`Lead ${lead.id} does not match criteria for journey ${journey.id}`);
                      }
                    } catch (error) {
                      console.error(`Error auto-enrolling lead ${lead.id} in journey ${journey.id}:`, error.message);
                    }
                  }
                  
                  if (journeyEnrolled > 0) {
                    console.log(`Enrolled ${journeyEnrolled} leads in journey "${journey.name}"`);
                  }
                }
                
                if (totalEnrolled > 0) {
                  console.log(`Total auto-enrollments completed: ${totalEnrolled} leads`);
                }
              } catch (error) {
            ...
            console.log('No auto-enroll journeys found for this tenant');
          }
        } catch (error) {
          console.error('Error checking for auto-enroll journeys:', error);
        }
      }
  ...
  app.get('/api/leads', authenticateToken, async (req, res) => {
    try {
      const { page = 1, limit = 50, status, phone, name, email, brand, source } = req.query;
      const tenantId = req.user.tenantId;
      
      const query = { where: { tenantId } };
      
      if (status) query.where.status = status;
      if (phone) query.where.phone = { [Op.iLike]: `%${phone}%` };
      if (name) query.where.name = { [Op.iLike]: `%${name}%` };
      if (email) query.where.email = { [Op.iLike]: `%${email}%` };
      ...
      // FIX: Pass only the where conditions to count()
      const count = await Lead.count({ where: query.where });
  ...
  app.delete('/api/leads/:id', authenticateToken, async (req, res) => {
    try {
      ...
      if (activeCalls) {
        return res.status(400).json({ error: 'Cannot delete lead with active calls' });
      }
  ...
  app.post('/api/leads/delete', authenticateToken, async (req, res) => {
    try {
      const { ids } = req.body;
      const tenantId = req.user.tenantId;
      ...
      if (activeCalls) {
        return res.status(400).json({ error: 'Cannot delete leads with active calls' });
      }
